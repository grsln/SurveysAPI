...
class SimpleSurveySerializer(serializers.ModelSerializer):
    ...
    simple_surveys_result = ResultSerializer(source="simple_surveys_result_set", many=True)

    class Meta:
        model = SimpleSurvey
        fields = ("id", "simple_survey_date", "status", "questions", "simple_surveys_result")
...
class SimpleSurveyResSerializer(serializers.ModelSerializer):
    ...
    simple_survey_result = ResultSerializer(source="simple_survey_result_set", many=True)

    class Meta:
        model = SimpleSurvey
        fields = ("id", "simple_survey_date", "status", "questions", "simple_survey_result")
        read_only_fields = ["id", "simple_survey_date", "status", "questions"]
...
class CustomUserEditSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        instance.answered_id = validated_data.get("username", instance.username)
        instance.answered_id = validated_data.get("password", instance.password)
        instance.save()
        return instance
